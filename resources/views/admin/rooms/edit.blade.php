<x-app-layout>
    ...
    <div class="py-6 flex justify-center">
        ...
        <div class="max-w-xl w-full bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 shadow-2xl">
            ...
            <!-- Form Pengiriman Data Menggunakan Method PUT untuk Update -->
            <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_kamar" placeholder="Contoh: Lavender, Tulip" required value="{{ old('nama_kamar', $room->nama_kamar) }}"
                ...
                <div>
                    ...
                    <select name="tipe_kamar" required 
                        ...
                        <option value="Standard" {{ old('tipe_kamar', $room->tipe_kamar) == 'Standard' ? 'selected' : '' }}>Standard Room</option>
                        <option value="Deluxe" {{ old('tipe_kamar', $room->tipe_kamar) == 'Deluxe' ? 'selected' : '' }}>Deluxe Room</option>
                        <option value="Suite" {{ old('tipe_kamar', $room->tipe_kamar) == 'Suite' ? 'selected' : '' }}>Suite Room</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="harga" placeholder="Contoh: 400000" required value="{{ old('harga', $room->harga) }}"
                        class="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-blue-500 transition text-base placeholder:text-slate-600 shadow-inner">
                </div>
                ...
                <div>
                    ...
                    <!-- Menampilkan Preview Foto yang Saat Ini Digunakan -->
                    @if($room->foto)
                        <div class="mb-3 flex items-center space-x-3 bg-slate-950/50 p-2.5 border border-white/5 rounded-xl">
                            <img src="{{ asset('storage/' . $room->foto) }}" alt="Foto Kamar" class="w-16 h-10 object-cover rounded-lg shadow">
                            <span class="text-xs text-slate-400">Foto saat ini aktif</span>
                        </div>
                    @endif
                    ...
                    <p class="text-[11px] text-slate-500 mt-1.5">*Kosongkan saja jika tidak ingin mengganti foto lama</p>
                </div>
                ...
                <div>
                    ...
                    <select name="status" required 
                        ...
                        <option value="penuh" {{ old('status', $room->status) == 'penuh' ? 'selected' : '' }}>Penuh / Dipesan</option>
                    </select>
                </div>

                <!-- TOMBOL AKSI: Ukuran tebal dan simetris, tombol Batal sudah diperbaiki kontrasnya -->
